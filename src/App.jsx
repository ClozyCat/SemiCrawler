...
function SettingsView() {
  const [form, setForm] = useState({ base_url: DASHSCOPE_BASE_URL, model_name: 'qwen3-max', api_key: '', request_headers: [], enabled: false })
  ...
  const addHeader = () => setForm({ ...form, request_headers: [...form.request_headers, { key: '', value: '' }] })
  const updateHeader = (index, field, value) => setForm({ ...form, request_headers: form.request_headers.map((header, itemIndex) => itemIndex === index ? { ...header, [field]: value } : header) })
  const removeHeader = (index) => setForm({ ...form, request_headers: form.request_headers.filter((_, itemIndex) => itemIndex !== index) })
  return <><div className="title-row"><div><span className="eyebrow">服务端配置</span><h1>API配置</h1><p>配置 OpenAI 兼容模型与 Dokobot 本地联网检索。</p></div></div><div className="settings-grid"><section className="panel model-form"><SlidersHorizontal /><form onSubmit={submit}><h2>AI 模型</h2><label className="field"><span>API 地址</span><input type="url" required value={form.base_url} onChange={(e) => setForm({ ...form, base_url: e.target.value })} /></label><label className="field"><span>模型名称</span><input required value={form.model_name} onChange={(e) => setForm({ ...form, model_name: e.target.value })} /></label><label className="field"><span>API Key {saved?.has_api_key && <small>已保存 {saved.api_key_hint}</small>}</span><input type="password" value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} placeholder={saved?.has_api_key ? '留空以保留现有密钥' : 'sk-...'} /></label><div className="header-config"><div className="header-config-title"><h3>请求头配置</h3><button type="button" className="secondary header-add" onClick={addHeader}><Plus />添加请求头</button></div>{form.request_headers.length > 0 && <div className="header-labels"><span>键</span><span>值</span><span /></div>}<div className="header-rows">{form.request_headers.map((header, index) => <div className="header-row" key={index}><input required aria-label={`请求头 ${index + 1} 键`} placeholder="例如 X-API-Version" value={header.key} onChange={(e) => updateHeader(index, 'key', e.target.value)} /><input required aria-label={`请求头 ${index + 1} 值`} placeholder="请求头值" value={header.value} onChange={(e) => updateHeader(index, 'value', e.target.value)} /><button type="button" className="icon-btn" title="删除请求头" aria-label={`删除请求头 ${index + 1}`} onClick={() => removeHeader(index)}><Trash2 /></button></div>)}</div></div><label className="check-field"><input type="checkbox" checked={form.enabled} onChange={(e) => setForm({ ...form, enabled: e.target.checked })} /><span>启用网站原文自动结构化</span></label><p className="field-note">模型会先为联网信息源规划搜索查询，并在 Dokobot 读取原文后进行结构化；上方开关只控制普通网站原文是否自动结构化。</p><p className="source-mode-note"><Globe2 />Dokobot 使用免费本地模式。需安装浏览器扩展与 CLI，并完成 <code>dokobot install-bridge</code>；系统不会调用 Remote 或保存 Dokobot API Key。</p>{error && <p className="form-error">{error}</p>}<button className="primary"><Save />保存模型设置</button></form></section></div></>
}
