...
function TaskPanel({ task, onLogs }) {
  ...
  return <section className="panel task-panel">
    <div className="panel-head"><div><h2>最近任务</h2><p>任务 #{task.id} · 起始日期 {task.start_date}</p></div><span className={`status ${task.status}`}>{taskStatusLabel(task.status)}</span></div>
    <div className="task-content">
      ...
      <div className="progress-meta"><span>{task.progress}%</span><span>{formatTime(task.completed_at || task.created_at)}</span></div>
