...
重启浏览器或重新加载扩展后，运行 `dokobot read --local https://dokobot.ai` 可验证本地桥接。添加信息源时选择“联网搜索”，填写自然语言检索主题及可选来源提示，并可从 Google、Bing、百度、搜狗和 360 搜索中指定首选搜索引擎。任务只在首选引擎无法连接时按 Google、Bing、百度、搜狗、360 搜索的默认顺序回退，然后用 `dokobot read --local` 搜索和读取原文，再用“API配置”中的 OpenAI 兼容模型生成结构化记录。来源提示中的每个 URL 会作为独立的 `site:` 搜索范围；Google 使用 `after:` 限定起始日期，百度使用搜索页时间参数，所有引擎的结果都会在保存前再次按任务起始日期过滤。每个规划关键词在每个来源范围内最多读取“查看数量上限”条结果，不设置跨关键词和跨来源的总数上限。任务执行全局串行，Dokobot 接管标签页上限为 5 个，并会复用和清理任务使用的标签页。
